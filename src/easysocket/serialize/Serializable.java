...
public interface Serializable {
	...
	public static int calculateSerializingSize(String[] array) {
		int size = 4;
		if (array != null) {
			for (String str : array) {
				size += calculateSerializingSize(str);
			}
		}
		return size;
	}

	public static <T extends Serializable> int calculateSerializingSize(T[] array) {
		int size = 4;
		if (array != null) {
			for (T t : array) {
				size += calculateSerializingSize(t);
			}
		}
		return size;
	}

	public static int calculateSerializingSize(byte[] array) {
		int size = 4;
		if (array != null) {
			size += array.length;
		}
